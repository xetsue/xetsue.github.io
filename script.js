			...
			document.addEventListener('DOMContentLoaded', function() {
			    ...
			    // This listener now ONLY handles the mobile case
			    ...
			    if (window.matchMedia("(min-width: 769px)").matches) {
                    document.querySelectorAll('.grid-item').forEach(item => {
                        ...
                        // Logic for hover effect (floating panel)
                        item.addEventListener('mouseenter', () => {
                            // Only show floating panel if not already expanded in-place
                        ...
                        item.addEventListener('mouseleave', () => {
                            ...
                            // Only reset border if it's not permanently expanded in-place
                            if (!description.classList.contains('expanded')) {
                                item.style.borderColor = '';
                            }
                        });

                        // Logic for toggle button click (in-place expansion)
                        toggleBtn.addEventListener('click', (e) => {
                            ...
                            if (isNowExpanded) {
                                // Set a persistent border color
                                item.style.borderColor = '#' + Math.floor(Math.random()*16777215).toString(16);
                                // Remove any temporary hover effect if it was active
                                item.classList.remove('is-expanded');
                            } else {
                                // Remove border color when collapsing
                                item.style.borderColor = '';
                            }
                        });
                    });
